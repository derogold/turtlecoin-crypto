...
void
tree_hash(const char (*hashes)[HASH_SIZE], size_t count, char *root_hash)
{
    assert(count > 0);
    if (count == 1)
    {
    ...
    } else
    {
        ...
        for (i = 2 * cnt - count, j = 2 * cnt - count; j < cnt;
             i += 2, ++j)
        {
...
void
tree_branch(const char (*hashes)[HASH_SIZE], size_t count,
            char (*branch)[HASH_SIZE])
{
    ...
    for (i = 2 * cnt - count, j = 2 * cnt - count - 1; j < cnt - 1;
         i += 2, ++j)
    {
        cn_fast_hash(hashes[i], 2 * HASH_SIZE, ints[j]);
    }
...
void
tree_hash_from_branch(const char (*branch)[HASH_SIZE], size_t depth,
                      const char *leaf, const void *path, char *root_hash)
{
    if (depth == 0)
    {
    ...
    } else
    {
        ...
        while (depth > 0)
        {
            --depth;
            if (path
                && (((const char *) path)[depth >> 3] & (1 << (depth & 7)))
                != 0)
            {
                leaf_path = buffer[1];
