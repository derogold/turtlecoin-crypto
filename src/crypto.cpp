...
#include "crypto-ops.h"
    ...
    static inline void random_scalar(EllipticCurveScalar & res) {
        unsigned char tmp[64];
         ...
         sc_reduce(tmp);
         memcpy(&res, tmp, 32);
    } static inline void hash_to_scalar(const void *data, size_t length,
    ...
    void crypto_ops::generate_deterministic_keys(PublicKey & pub,
                                                 SecretKey & sec,
                                                 SecretKey & second) {
        lock_guard < mutex > lock(random_lock);
        ge_p3 point;
    ...
    SecretKey crypto_ops::generate_m_keys(PublicKey & pub, SecretKey & sec,
    ...
    bool crypto_ops::generate_key_derivation(const PublicKey & key1,
        ...
        if (ge_frombytes_vartime
            (&point,
             reinterpret_cast < const unsigned char *>(&key1)) != 0)
        {
            return false;
        }
    ...
    static void derivation_to_scalar(const KeyDerivation & derivation,
                                     size_t output_index,
                                     EllipticCurveScalar & res) {
        struct {
            KeyDerivation derivation;
    ...
    bool crypto_ops::derive_public_key(const KeyDerivation & derivation,
                                       size_t output_index,
        ...
        ge_scalarmult_base(&point2,
                           reinterpret_cast < unsigned char *>(&scalar));
        ge_p3_to_cached(&point3, &point2);
        ge_add(&point4, &point1, &point3);
        ge_p1p1_to_p2(&point5, &point4);
        ge_tobytes(reinterpret_cast < unsigned char *>(&derived_key),
                   &point5);
        return true;
    }
    ...
    bool crypto_ops::underive_public_key_and_get_scalar(const KeyDerivation
                                                        & derivation,
                                                        size_t
                                                        output_index,
                                                        const PublicKey &
                                                        ...
                                                        hashed_derivation)
    {
        ...
        ge_scalarmult_base(&point2,
                           reinterpret_cast <
                           unsigned char *>(&hashed_derivation));
        ge_p3_to_cached(&point3, &point2);
    ...
    void crypto_ops::derive_secret_key(const KeyDerivation & derivation,
                                       size_t output_index,
    ...
    bool crypto_ops::underive_public_key(const KeyDerivation & derivation,
                                         size_t output_index,
    ...
    void crypto_ops::generate_signature(const Hash & prefix_hash,
                                        const PublicKey & pub,
                                        const SecretKey & sec,
                                        Signature & sig) {
        lock_guard < mutex > lock(random_lock);
        ...
        hash_to_scalar(&buf, sizeof(s_comm),
                       reinterpret_cast < EllipticCurveScalar & >(sig));
    ...
    bool crypto_ops::check_signature(const Hash & prefix_hash,
                                     const PublicKey & pub,
                                     const Signature & sig) {
        ge_p2 tmp2;
        ...
        if (sc_check(reinterpret_cast < const unsigned char *>(&sig)) != 0
            || sc_check(reinterpret_cast <
                        const unsigned char *>(&sig) + 32) != 0)
        {
            return false;
        }
    ...
    void crypto_ops::generate_key_image(const PublicKey & pub,
                                        const SecretKey & sec,
                                        KeyImage & image) {
        ge_p3 point;
...
#pragma warning(disable: 4200)
    ...
    bool crypto_ops::generateRingSignatures(const Hash
        ...
        rs_comm *const buf =
            reinterpret_cast <
            rs_comm * >(alloca(rs_comm_size(publicKeys.size())));
        ...
        for (size_t i = 0; i < publicKeys.size(); i++)
        {
            ...
            {
                random_scalar(k);
                ge_scalarmult_base(&tmp3,
                                   reinterpret_cast <
                                   unsigned char *>(&k));
                ge_p3_tobytes(reinterpret_cast <
                              unsigned char *>(&buf->ab[i].a), &tmp3);
                hash_to_ec(publicKeys[i], tmp3);
                ge_scalarmult(&tmp2,
                              reinterpret_cast < unsigned char *>(&k),
                              &tmp3);
                ge_tobytes(reinterpret_cast <
                           unsigned char *>(&buf->ab[i].b), &tmp2);
            } else
            {
                random_scalar(reinterpret_cast <
                              EllipticCurveScalar & >(signatures[i]));
                random_scalar(*reinterpret_cast <
                              EllipticCurveScalar * >(reinterpret_cast <
                                                      unsigned char
                                                      *>(&signatures[i]) +
                                                      32));
                ...
                sc_add(reinterpret_cast < unsigned char *>(&sum),
                       ...
                       reinterpret_cast <
                       unsigned char *>(&signatures[i]));
            }
        }

        hash_to_scalar(buf, rs_comm_size(publicKeys.size()), h);

        sc_sub(reinterpret_cast <
               unsigned char *>(&signatures[realOutput]),
               reinterpret_cast < unsigned char *>(&h),
               reinterpret_cast < unsigned char *>(&sum));

        sc_mulsub(reinterpret_cast <
                  unsigned char *>(&signatures[realOutput]) + 32,
                  reinterpret_cast <
                  unsigned char *>(&signatures[realOutput]),
                  reinterpret_cast <
    ...
    bool crypto_ops::checkRingSignature(const Hash & prefix_hash,
        ...
        rs_comm *const buf =
            reinterpret_cast <
            rs_comm * >(alloca(rs_comm_size(pubs.size())));
        ...
        for (size_t i = 0; i < pubs.size(); i++)
        {
            ...
            if (sc_check
                (reinterpret_cast <
                 const unsigned char *>(&signatures[i])) != 0
                || sc_check(reinterpret_cast <
                            const unsigned char *>(&signatures[i]) + 32) !=
                0)
            {
                return false;
            }
            ...
            ge_double_scalarmult_precomp_vartime(&tmp2, reinterpret_cast < const unsigned char
                                                 *>(&signatures[i]) + 32,
                                                 &tmp3,
                                                 reinterpret_cast <
                                                 const unsigned char
            ...
            ge_tobytes(reinterpret_cast < unsigned char *>(&buf->ab[i].b),
                       &tmp2);

            sc_add(reinterpret_cast < unsigned char *>(&sum),
                   reinterpret_cast < unsigned char *>(&sum),
                   reinterpret_cast <
                   const unsigned char *>(&signatures[i]));
        }

        hash_to_scalar(buf, rs_comm_size(pubs.size()), h);

        sc_sub(reinterpret_cast < unsigned char *>(&h),
               reinterpret_cast < unsigned char *>(&h),
               reinterpret_cast < unsigned char *>(&sum));
